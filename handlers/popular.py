...
async def popular_games_callback(callback: CallbackQuery):
    ...
    try:
        ...
        log_error(f"Ошибка при отображении сообщения о загрузке популярных игр для пользователя {user_id}: {str(e)}")
...
async def show_popular_games(message, original_user_id=None, username=None):
    ...
    try:
        ...
        if not games:
            ...
            if user_id:
                log_error(f"Не удалось загрузить популярные игры для пользователя {user_id}")
        ...
        if user_id:
            log_info(f"Загружены популярные игры для пользователя {user_id}: {', '.join(game_names[:3])}")
        ...
        try:
            ...
            if user_id:
                log_error(f"Ошибка при отображении популярных игр для пользователя {user_id}: {str(e)}")
        ...
        if user_id:
            log_error(f"Ошибка при загрузке популярных игр для пользователя {user_id}: {str(e)}")
