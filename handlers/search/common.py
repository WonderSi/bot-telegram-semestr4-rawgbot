...
async def search_games(message: Message, query: str, index: int = 0, search_type: str = 'name', original_user_id=None, original_username=None):
    ...
    if index == 0:
        ...
        log_info(
            f"Начат поиск игр для пользователя {user_id}: '{query}', тип: {search_type}")
    else:
        log_info(
            f"Навигация по результатам поиска для пользователя {user_id}: '{query}', индекс: {index}")
    ...
    try:
        ...
        if not games or index >= len(games):
            if index == 0:
                ...
                log_info(
                    f"Поиск для пользователя {user_id} не дал результатов: '{query}', тип: {search_type}")
                ...
                log_info(
                    f"Достигнут конец результатов для пользователя {user_id}: '{query}', индекс: {index}")
        ...
        log_info(
            f"Поиск для пользователя {user_id} дал {len(games)} результатов: '{query}', тип: {search_type}")
        log_user(user_id, username,
                 f"Просматривает игру '{name}' (ID: {game_id}), результат {index + 1} из {len(games)}")
        ...
        try:
            if image_url:
                await message.answer_photo(
                    ...
                    reply_markup=get_game_menu(
                        game_id, query, index, len(games), search_type),
                    parse_mode='Markdown'
                )
            ...
            log_error(
                f"Ошибка при отображении игры {name} для пользователя {user_id}: {str(e)}")
            await message.answer(
                text=result_text,
                reply_markup=get_game_menu(
                    game_id, query, index, len(games), search_type),
        ...
        log_error(
            f"Ошибка при поиске игр для пользователя {user_id}: '{query}', тип '{search_type}' - {str(e)}")
