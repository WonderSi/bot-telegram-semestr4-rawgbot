    ...
    try:
        ...
        if index < max_results - 1:
            log_user(user_id, username,
                     f"Запросил следующий результат поиска '{query}', индекс {index+1}")
            ...
            log_info(
                f"Пользователь {user_id} достиг конца результатов поиска '{query}'")
            await callback.answer("Больше результатов нет", show_alert=True)
    except Exception as e:
        log_error(
            f"Ошибка при навигации по результатам поиска для пользователя {user_id}: {str(e)}")
