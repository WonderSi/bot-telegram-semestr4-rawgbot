...
async def details_search_game_callback(callback: CallbackQuery):
    ...
    try:
        ...
        log_user(user_id, username, f"Запросил детали игры с ID {game_id} (из поиска)")
        ...
        log_error(f"Ошибка при обработке запроса деталей игры (из поиска) для пользователя {user_id}: {str(e)}")
...
async def details_popular_game_callback(callback: CallbackQuery):
    ...
    try:
        ...
        log_user(user_id, username, f"Запросил детали игры с ID {game_id} (из популярных)")
        await callback.answer("Загружаю подробную информацию...")
        ...
        log_error(f"Ошибка при обработке запроса деталей игры (из популярных) для пользователя {user_id}: {str(e)}")
        await callback.message.answer(
            "Произошла ошибка при получении информации об игре",
            reply_markup=get_back_button()
        )
...
async def details_random_game_callback(callback: CallbackQuery):
    ...
    try:
        ...
        log_user(user_id, username, f"Запросил детали игры с ID {game_id} (из случайных)")
        await callback.answer("Загружаю подробную информацию...")
        ...
        log_error(f"Ошибка при обработке запроса деталей игры (из случайных) для пользователя {user_id}: {str(e)}")
        await callback.message.answer(
            "Произошла ошибка при получении информации об игре",
            reply_markup=get_back_button()
        )
...
async def show_game_details(callback: CallbackQuery, game_id: int, source_type: str, original_user_id=None, username=None):
    ...
    try:
        log_info(f"Загрузка деталей игры {game_id} для пользователя {user_id} (тип: {source_type})")
        log_user(user_id, current_username, f"Запросил детали игры (ID: {game_id})")
        
        game_details = await rawg_client.get_game_details(game_id)
        
        if not game_details:
            log_error(f"Не удалось получить информацию об игре {game_id} для пользователя {user_id}")
        ...
        description_raw = game_details.get('description_raw', 'Описание отсутствует')
        description = description_raw[:500] + "..." if len(description_raw) > 500 else description_raw
        ...
        log_error(f"Ошибка при получении деталей игры {game_id} для пользователя {user_id}: {str(e)}")
