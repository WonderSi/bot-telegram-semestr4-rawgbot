...
async def random_game_callback(callback: CallbackQuery):
    ...
    try:
        ...
        log_error(
            f"Ошибка при отображении сообщения о загрузке для пользователя {user_id}: {str(e)}")
...
async def back_to_random_callback(callback: CallbackQuery):
    ...
    try:
        ...
        if last_random_game:
            log_info(
                f"Показываем кэшированную случайную игру для пользователя {user_id}")
            await show_random_game_from_data(callback.message, last_random_game, original_user_id=user_id, username=username)
        else:
            log_info(
                f"Кэш пуст, ищем новую случайную игру для пользователя {user_id}")
            loading_message = await callback.message.answer("🎲 Ищу случайную игру...")
            log_bot(user_id, "🎲 Ищу случайную игру...")
            await show_random_game(loading_message, original_user_id=user_id, username=username)
    except Exception as e:
        log_error(
            f"Ошибка при возврате к случайной игре для пользователя {user_id}: {str(e)}")
        await callback.message.answer(
            "Произошла ошибка при возврате к игре. Попробуйте найти новую случайную игру.",
            reply_markup=get_back_button()
        )
        log_bot(
            user_id, "Произошла ошибка при возврате к игре. Попробуйте найти новую случайную игру.")
...
async def show_random_game_from_data(message, game, original_user_id=None, username=None):
    ...
    user_id = original_user_id or (
        message.from_user.id if hasattr(message, 'from_user') else None)
    ...
    try:
        ...
        if user_id:
            log_info(
                f"Пользователь {user_id} просматривает случайную игру '{name}' (ID: {game_id})")
            if username:
                log_user(user_id, username,
                         f"Просматривает случайную игру '{name}'")
        ...
        if user_id:
            log_error(
                f"Ошибка при отображении случайной игры для пользователя {user_id}: {str(e)}")
        await message.answer(
        ...
        if user_id:
            log_bot(
                user_id, "Произошла ошибка при отображении игры. Попробуйте найти новую случайную игру.")
...
async def show_random_game(message, original_user_id=None, username=None):
    ...
    try:
        ...
        if not game:
            ...
            if user_id:
                log_error(
                    f"Не удалось найти случайную игру для пользователя {user_id}")
        ...
        if user_id:
            log_info(
                f"Найдена случайная игра '{game.get('name', 'Нет названия')}' для пользователя {user_id}")
        ...
        if user_id:
            log_error(
                f"Ошибка при поиске случайной игры для пользователя {user_id}: {str(e)}")
